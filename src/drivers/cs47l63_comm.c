...
LOG_MODULE_REGISTER(CS47L63, LOG_LEVEL_INF);
...
int cs47l63_comm_init(cs47l63_t *cs47l63_driver)
{
	...
	if (ret) {
		LOG_ERR("HW Codec GPIO failed with error code: %d", ret);
		return ret;
	}
	...
	if (ret) {
		LOG_ERR("HW Codec IRQ failed with error code: %d", ret);
		return ret;
	}
	...
	if (ret) {
		LOG_ERR("HW Codec reset failed with error code: %d", ret);
		return ret;
	}
	...
	// if (ret) {
	// 	LOG_ERR("Thread name set failed with error code: %d", ret);
	// 	return ret;
	// }
