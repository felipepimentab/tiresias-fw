...
#define FRAME_SIZE_BYTES ((CONFIG_I2S_LRCK_FREQ_HZ / 1000 * 15 / 2) * CONFIG_I2S_CH_NUM * CONFIG_AUDIO_BIT_DEPTH_OCTETS)
#else
 ...
 */
typedef void (*i2s_blk_comp_callback_t)(
    uint32_t frame_start_ts, uint32_t* rx_buf_released, uint32_t const* tx_buf_released);
